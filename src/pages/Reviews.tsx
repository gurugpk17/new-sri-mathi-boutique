  ...
  if (loading) {
    return (
      <div className="pt-40 min-h-screen flex flex-col items-center justify-center text-gold space-y-4">
        ...
        <p className="font-accent text-[10px] uppercase tracking-widest">Gathering Kind Words...</p>
      </div>
    );
  }
        ...
        {featured && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
            <div className="lg:col-span-7 bg-neutral-900 border border-white/5 overflow-hidden group">
              <img 
                src={featured.image || "https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&q=80&w=800"} 
                alt={featured.name}
            ...
            <div className="lg:col-span-5 space-y-8">
              ...
              <div>
                <p className="font-accent text-gold font-bold">{featured.name}</p>
                <p className="font-accent text-gray-500 text-[10px] tracking-[0.3em] uppercase mt-2">{featured.location}</p>
              </div>
            </div>
          </div>
        )}
            ...
            <React.Fragment key={rev.id}>
              <div className="bg-neutral-900 border border-gold/10 p-12 flex flex-col justify-between">
                ...
                <div className="mt-12 text-left">
                  <p className="font-accent text-gold text-xs font-bold uppercase tracking-widest">{rev.name}</p>
                  <p className="font-accent text-gray-500 text-[9px] tracking-[0.4em] uppercase mt-1">{rev.location}</p>
                </div>
              </div>
              {i === 0 && (
